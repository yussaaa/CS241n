...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            W_i = self.stack.pop(-2)
            W_j = self.stack[-1]
            self.dependencies.append((W_j, W_i))
        elif transition == "RA":
            W_j = self.stack[-2]
            W_i = self.stack.pop(-1)
            self.dependencies.append((W_j, W_i))
    ...
    def is_completed(self):
        return len(self.stack) == 1 and self.buffer == []
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        # Select the batch_size in the unfinished_parses as the minibatch
        minibatch = unfinished_parses[:batch_size]
        transitions = model.predict(minibatch)
        # Apply predict transition to each of the minibatch
        ...
        unfinished_parses[:] = \
            [p for p in unfinished_parses if not ((len(p.stack) == 1) and (len(p.buffer) == 0))]
