...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 1: Embed the padded source
        X = self.model_embeddings_source(source_padded)

        # 2: pack the padded sequences
        ...
        # 3:
        ## First, consider init_decoder_hidden
        h_fwd, h_bwd = last_hidden[0], last_hidden[1]
        h_enc_act = torch.cat([h_fwd, h_bwd], dim=1)
        init_decoder_hidden = self.h_projection(h_enc_act)

        c_fwd, c_bwd = last_cell[0], last_cell[1]
        c_enc_act = torch.cat([c_fwd, c_bwd], dim=1)
        init_decoder_cell = self.h_projection(c_enc_act)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y,1,dim=0):
            ...
            o_prev = o_t
        #4: Change the list optput to a tensor size of (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 2: Split the hidden and the cell states
        ...
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        # 1. Softmax to get the attention distribuion
        alpha_t = nn.functional.softmax(e_t, dim=1) #(b, src_len)
        alpha_t = torch.unsqueeze(alpha_t,dim=1) #change dimension (b, src_len, 2h)

        ## 2. Weighted sum of the encoder hidden state and attention weight
        a_t = torch.bmm(alpha_t,enc_hiddens)
        a_t = torch.squeeze(a_t, dim=1) #(b, 2h)

        ## 3.Concatenate attention output and decoder hidden state
        U_t = torch.cat([dec_hidden,a_t], dim=1)

        ## 4.Output linear leayer
        V_t = self.combined_output_projection(U_t)

        ##5. Output dropout layer
