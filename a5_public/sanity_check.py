...
def question_1h_sanity_check():
    """Sanity check for question 1h highway layer output
    TODO:
        1. Input and output have the expected shape and types (must have the clear docstring first)
        2. print out the shape of every intermediate value.
        3. Manually create small instance to check
            print all the intermediate values
    """
    print ("-"*80)
    print("Running Sanity Check for Question 1h: HighWay Layer")
    print ("-"*80)

    input = torch.ones(BATCH_SIZE * 2, EMBED_SIZE)
    highway = Highway(input_size=EMBED_SIZE)
...
def question_1i_sanity_check():
    ...
    ## Specify the sample dimention
    b=5 #Batch size
    e_char=50 #Character embedding size 50
    e_word=256  #Word embedding size 256
    max_word=21  #Maximum word length 21
    max_sents=2  #Maximum sentence length: max_sent_length

    input = torch.ones(b*max_sents, e_char, e_word)
    cnn = CNN(input_channels=e_char, output_channels=e_word)
    x_conv_out = cnn(input)
    assert x_conv_out.shape[0] == max_sents * \
           b, "output shape is incorrect: it should be:\n {} but is: \n {}".format(
        input.shape, x_conv_out.shape)
    assert x_conv_out.shape[1] == e_word, "Wrong embedding word size (channel dim)"
