...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        def startend(c):
            c.insert(0, self.start_of_word)
            c.append(self.end_of_word)
            return c
        return [[startend([self.char2id[char] for char in w]) for w in s] for s in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        word_char_ids = self.words2charindices(sents)
        sents_t = pad_sents_char(word_char_ids, self['<pad>'])
